...
//String host = "http://192.168.101.137"
//String host = "http://192.168.1.12"
//String arm = "http://192.168.43.148:8081"
//String arm = "http://192.168.101.137:8081"
...
// *****************************************************************
// ***   Retrieve Get CxOSA scan details for the specified CxOSA scan Id       ***
// *****************************************************************

def resultsReady = "InProgress"

while(resultsReady == "InProgress")
{
    sleep(15000)

    println("\n Get CxOSA scan details for the specified CxOSA for " + scanIDval + " " + resultsReady )
    println("/cxrestapi/osa/scans/${scanIDval}")
    try{
        site.get(path: "/cxrestapi/osa/scans/${scanIDval}",
                headers: ['Authorization': "Bearer ${accessToken}", 'requestContentType': "JSON"]  )
                { resp2, json ->
                    def String rVal = ""

                    state = json['findingsStatus']
                //    println(state)
                    state2 = json['state']
               //     println(state2)
                    resultsReady = state2['name']
                    println(resultsReady)
